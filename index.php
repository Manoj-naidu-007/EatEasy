...
<body class="home">
  ...
  <img class="img-rounded" src="images/eateasy1.png" alt="EatEasy Logo"
       style="width: 60px;height: 60px;">
                    ...
                    <div class="collapse navbar-toggleable-md  float-lg-right" id="mainNavbarCollapse">
                        <ul class="nav navbar-nav">
                            ...
                            <li class="nav-item"> <a class="nav-link active" href="./restaurants/restaurant_auth.php">Restaurant login<span class="sr-only"></span></a> </li>
                             <li class="nav-item"> <a class="nav-link active" href="./delivery/dashboard.php">Delivery login<span class="sr-only"></span></a> </li> 
                            
                           
							<?php
						if(empty($_SESSION["user_id"])) // if user is not login
							{
								echo '<li class="nav-item"><a href="login.php" class="nav-link active">User Login</a> </li>';
							}
						else
							{
...
<div id="chatbot">
  ...
  <div class="chat-body" id="chatBody">
    <div class="bot-msg">👋 Welcome to EatEasy</div>
    <div class="bot-msg">Choose an option:</div>

    <div class="quick-buttons">
      <button onclick="loadRestaurants()">🍽 Restaurants</button>
      <button onclick="loadRestaurants()">📋 Menu</button>
...
function loadRestaurants() {
  appendMessage("Loading restaurants 🍽...", "bot");

  fetch("chatbot_api.php?action=restaurants")
    .then(res => res.json())
    .then(data => {
      let html = '<div class="quick-buttons">';
      data.forEach(r => {
        html += `<button onclick="loadMenu(${r.rs_id})">${r.title}</button>`;
      });
      html += '</div>';
...
function loadMenu(id) {
  appendMessage("Loading menu 📋...", "bot");

  fetch(`chatbot_api.php?action=menu&res_id=${id}`)
    .then(res => res.json())
    .then(data => {
      let html = '<div class="quick-buttons">';
      data.forEach(d => {
        html += `<button onclick="addToCart(${d.d_id}, '${d.title}', ${d.price})">
          ${d.title} ₹${d.price}
        </button>`;
      });
      html += '</div>';
...
/* ================= RENDER CART ================= */
function renderCart() {
  let html = `<div class="bot-msg"><b>🛒 Your Cart</b></div>`;

  if (cart.length === 0) {
    html += `<div class="bot-msg">Cart is empty 😔</div>`;
    chatBody.innerHTML += html;
    return;
  }
  ...
  html += `
    <div class="bot-msg"><b>Total: ₹${total}</b></div>
    <div class="quick-buttons">
...
function clearCart() {
  cart = [];
  appendMessage("Cart cleared ❌", "bot");
}

/* ================= PLACE ORDER ================= */
function placeOrder() {
  fetch("chatbot_api.php?action=save_cart", {
    method: "POST",
...
function sendMessage() {
  ...
  if (msg.includes("restaurant")) {
    ...
    appendMessage("Try: restaurant, menu, order 😊", "bot");
  }
}
